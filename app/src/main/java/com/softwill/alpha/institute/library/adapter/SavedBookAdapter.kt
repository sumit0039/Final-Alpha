...
import com.softwill.alpha.databinding.ItemSavedBookBinding
...
class SavedBookAdapter(
    ...
    RecyclerView.Adapter<SavedBookAdapter.ViewHolder>() {
    override fun onCreateViewHolder(
        parent: ViewGroup, viewType: Int
    ): SavedBookAdapter.ViewHolder {
        val binding: ItemSavedBookBinding = DataBindingUtil.inflate(
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        with(holder) {
            with(mList[position]) {
                ...
                Glide.with(context).load(thumbnailUrl).placeholder(R.drawable.icon_no_image) .into(binding.ivImage)

                binding.tvBookTitle.text = name


                binding.ivSaved.setOnClickListener {
                    apiSaveRemoveBook(id,  position)
                }
    ...
    inner class ViewHolder(val binding: ItemSavedBookBinding) :
    ...
    private fun apiSaveRemoveBook(id: Int,  position: Int) {

        val jsonObject = JsonObject().apply {
            addProperty("status", "remove")
        }
            ...
            override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                if (response.isSuccessful) {
                    ...
                    if (responseObject.has("message")) {
                        ...
                        if (message == "Remove successfully") {
                            removeItem(position)
                        }
