...
import com.softwill.alpha.databinding.ItemLibraryRecentBinding
...
class LibraryRecentAdapter(
    private val context: Context,
    private var mList: ArrayList<Book>,
) :
    RecyclerView.Adapter<LibraryRecentAdapter.ViewHolder>() {
    override fun onCreateViewHolder(
        parent: ViewGroup, viewType: Int
    ): LibraryRecentAdapter.ViewHolder {
        val binding: ItemLibraryRecentBinding = DataBindingUtil.inflate(
            LayoutInflater.from(parent.context),
            R.layout.item_library_recent,
            parent,
            false
        )
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        with(holder) {
            with(mList[position]) {
                ...
                if (isSaved == 1) {
                    binding.ivSaved.setImageResource(R.drawable.icon_saved)
                } else {
                    binding.ivSaved.setImageResource(R.drawable.icon_save)
                }
    ...
    inner class ViewHolder(val binding: ItemLibraryRecentBinding) :
